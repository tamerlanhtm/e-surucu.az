    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('admin.headlines.update', $headline) }}" method="POST">
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <label for="title" class="block text-sm font-medium text-gray-700 mb-2">Başlıq Mətni</label>
                            <input type="text" name="title" id="title" value="{{ old('title', $headline->title) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="url" name="link" id="link" value="{{ old('link', $headline->link) }}"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="order" id="order" value="{{ old('order', $headline->order) }}" min="0"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                            @error('order')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4 flex items-center">
                            <input type="checkbox" name="open_in_new_tab" id="open_in_new_tab" value="1" {{ old('open_in_new_tab', $headline->open_in_new_tab) ? 'checked' : '' }}
                        ...
                        <div class="mb-4 flex items-center">
                            <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $headline->is_active) ? 'checked' : '' }}
                                class="rounded border-gray-300 text-primary shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
