<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Reklamı Redaktə Et: {{ $advertisement->name }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-4">
                <div class="p-4">
                    <h3 class="text-sm font-medium text-gray-700 mb-3">Statistika</h3>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="bg-blue-50 rounded-lg p-3">
                            <div class="text-2xl font-bold text-blue-600">{{ number_format($advertisement->impressions) }}</div>
                            <div class="text-xs text-blue-800">Göstərim</div>
                        </div>
                        <div class="bg-green-50 rounded-lg p-3">
                            <div class="text-2xl font-bold text-green-600">{{ number_format($advertisement->clicks) }}</div>
                            <div class="text-xs text-green-800">Klik</div>
                        </div>
                        <div class="bg-purple-50 rounded-lg p-3">
                            <div class="text-2xl font-bold text-purple-600">{{ $advertisement->ctr }}%</div>
                            <div class="text-xs text-purple-800">CTR</div>
                        </div>
                        <div class="flex items-center justify-center">
                            <form action="{{ route('admin.advertisements.reset-stats', $advertisement) }}" method="POST" onsubmit="return confirm('Statistikanı sıfırlamaq istədiyinizdən əminsiniz?')">
                                @csrf
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('admin.advertisements.update', $advertisement) }}" method="POST" enctype="multipart/form-data" x-data="{
                        type: '{{ old('type', $advertisement->type) }}',
                        position: '{{ old('position', $advertisement->position) }}',
                        positionInfo: {
                            'header': {
                                title: 'Header (Yuxarı)',
                            ...
                            <div class="lg:col-span-2 space-y-4">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" name="name" id="name" value="{{ old('name', $advertisement->name) }}"
                                    ...
                                    <div>
                                        ...
                                        <select name="position" id="position" x-model="position"
                                            ...
                                            @foreach($positions as $key => $label)
                                                <option value="{{ $key }}" {{ old('position', $advertisement->position) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                            @endforeach
                                        </select>
                                        @error('position')
                                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        ...
                        <div x-show="type === 'image'" class="mb-4 p-4 bg-gray-50 rounded-lg">
                            @if($advertisement->image)
                                <div class="mb-4">
                                    ...
                                    <img src="{{ Storage::url($advertisement->image) }}" alt="{{ $advertisement->alt_text }}" class="h-24 w-auto rounded border">
                                </div>
                            @endif

                            <div class="mb-4">
                                <label for="image" class="block text-sm font-medium text-gray-700 mb-2">Yeni Şəkil</label>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="alt_text" id="alt_text" value="{{ old('alt_text', $advertisement->alt_text) }}"
                                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50"
                            ...
                            <div class="mt-4 flex items-center">
                                <input type="checkbox" name="open_in_new_tab" id="open_in_new_tab" value="1" {{ old('open_in_new_tab', $advertisement->open_in_new_tab) ? 'checked' : '' }}
                        ...
                        <div x-show="type === 'code'" class="mb-4 p-4 bg-gray-50 rounded-lg">
                            <div>
                                ...
                                <textarea name="code" id="code" rows="8"
                                    ...
                                    placeholder="Google AdSense kodu və ya xüsusi HTML/JavaScript">{{ old('code', $advertisement->code) }}</textarea>
                                @error('code')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="border-t pt-6 mt-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $advertisement->start_date?->format('Y-m-d')) }}"
                                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                                    @error('start_date')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $advertisement->end_date?->format('Y-m-d')) }}"
                                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                                    @error('end_date')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                        ...
                        <div class="border-t pt-6 mt-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" name="order" id="order" value="{{ old('order', $advertisement->order) }}"
                                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                                    @error('order')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                ...
                                <div class="flex items-center h-full pt-6">
                                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $advertisement->is_active) ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-primary shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
